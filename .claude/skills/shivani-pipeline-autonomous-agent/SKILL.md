...
Two systemd `--user` daemons (`shivani-telegram-listener`, `shivani-pipeline-orchestrator`) read from `/etc/shivani-pipeline/agent.env`, share state through `ops/shivani/work-queue.db` (SQLite WAL), and spawn one `claude -p` per URL using model `${CLAUDE_MODEL:-claude-sonnet-4-6}` with adaptive thinking (`--effort ${CLAUDE_EFFORT:-xhigh}`). Allowlist is single-user; URLs from any other Telegram user are silently ignored. Daily cap defaults to **50** (`CAP_DAILY_MAX`), weekly to **250** (`CAP_WEEKLY_MAX`). **The rate-limit state file at `${RATE_LIMIT_STATE_PATH:-/var/lib/claude-pipeline/rate-limit.json}` is SHARED with the Yash daemon** — both bots share the same Claude Max login on this VPS, so a usage-limit hit by either tenant pauses both queues until the 5-hour window resets; the URL is re-queued without consuming a retry attempt. Per-URL latency ~13–25 min (V3.1 prompt + cover-letter generation + tectonic compile). The per-URL wall-clock timeout is **35 min** (configurable via `PER_URL_TIMEOUT_MS`); a run that finishes is credited even if SIGTERM'd at the timeout — success is gated on artifacts-on-disk, not the `claude -p` exit code (fixed 2026-05-29). Plan: `docs/superpowers/plans/2026-05-25-shivani-autonomous-agent-impl-plan.md`.
